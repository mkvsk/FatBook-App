...
/**
 * Library was not used as dependency for customization needs
 * All credits to https://github.com/MikeOrtiz/TouchImageView
 */
...
open class TouchImageView @JvmOverloads constructor(
    ...
    private fun getFixTrans(
        trans: Float, viewSize: Float, contentSize: Float, offset: Float
    ): Float {
        val minTrans: Float
        val maxTrans: Float
    ...
    private fun fitImageToView() {
        ...
        @Suppress("SENSELESS_COMPARISON") if (touchMatrix == null || prevMatrix == null) {
            return
        }
        if (userSpecifiedMinScale == AUTOMATIC_MIN_ZOOM) {
            minZoom = AUTOMATIC_MIN_ZOOM
    ...
    fun setZoomAnimated(
        scale: Float, focusX: Float, focusY: Float, listener: OnZoomFinishedListener?
    ) {
